 ...
 * Time:	2019年03月13日 星期三 19时34分53秒
 ************************************************************************/
...
struct my_node {
 ...
 int my_insert(struct rb_root *root, int key)
 {
     ...
     if ((mynode=malloc(sizeof(struct my_node))) == NULL)
         return -1; 
    mynode->key = key;
	mynode->data = (char*)malloc(10*sizeof(char));
	sprintf(mynode->data,"index=%d",key);
 ...
 static void print_rbtree(struct rb_node *tree, int key, int direction)
 {
     if(tree != NULL)
     {   
		struct my_node *my = container_of(tree, struct my_node, rb_node);
         if(direction==0)    // tree是根节点
             printf("%2d(B)(%s) is root\n", key,my->data);
         else                // tree是分支节点
             printf("%2d(%s)(%s) is %2d's %6s child\n", key, rb_is_black(tree)?"B":"R",my->data, key, direction==1?"right" : "left");
