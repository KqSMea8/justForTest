...
int base64_decode(char *out, int max_outlen,const char *in, int inlen) {
    int outlen = 0;
    int over = 0; // decode over flag
    register char digit0, digit1, digit2, digit3;
    char *out_org = out;
    memset(out, 0x00, max_outlen);
    ...
    for (; inlen >= 4; inlen -= 4, in += 4) {
        ...
        if (++outlen >= max_outlen && max_outlen) {
            *out_org = '\0';
...
int base64_encode(char *out, int max_outlen,const char *in, int inlen) {
    int outlen = 0;
    char *out_org = out;
    memset(out, 0x00, max_outlen);
