...
static void make_digest_ex(char *md5str, const unsigned char *digest, int len) {
...
static void make_digest(char *md5str, const unsigned char *digest) {
...
static void MD5Update(MD5_CTX *ctx, const void *data, size_t size) {
    uint32_t saved_lo;
    uint32_t used, free;
    saved_lo = ctx->lo;
...
static void MD5Final(unsigned char *result, MD5_CTX *ctx) {
    uint32_t used, free;
    used = ctx->lo & 0x3f;
 ...
 * 生成文件的MD5值的原始值
 */
int MD5FileRaw(const void *file, unsigned char *output, size_t outputLen) {
    if (outputLen < 16) {
        return -1;
    }
